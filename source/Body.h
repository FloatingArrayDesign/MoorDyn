...
class Body
{
	...
	// unique to Body:
	moordyn::Connection*
	    attachedC[30]; // pointers to connections attached to this body
	...
	// input file has	Name/ID      X0   Y0   Z0   Xcg   Ycg   Zcg      M V IX
	// IY       IZ     CdA-x,y,z Ca-x,y,z

	// degrees of freedom (or states)
	double r6[6]; // body 6dof position [x/y/z] // double* to continuous state
	              // or input state or parameter/other-state
	double v6[6]; // body 6dof velocity[x/y/z]  // double* to continuous state
	              // or input state or parameter/other-state
	...
	double M[6]
	        [6]; // total body mass + added mass matrix including all elements
	double M0[6][6]; // starting mass and added mass matrix (6x6) of body
	                 // without any rod elements in inertital orientation

	double OrMat[9]; // orientation matrix of body (rotation matrix that gets it
  ...
  public:
	...
	void addConnectionToBody(moordyn::Connection* theConnection,
	                         double coords[3]);

	void addRodToBody(Rod* theRod, double endCoords[6]);

	void initializeUnfreeBody(const double r_in[6],
