            ...
            double* yout)
{
	...
	for (int i = 0; i < n; i++) {
		...
		if (id > ndata)
			yout[i] = ydata[ndata - 1]; // quick fix: for now if we go past the
			                            // data just use the last value
		else
			yout[i] = ydata[id - 1] + (xin[i] - xdata[id - 1]) /
			                              (xdata[id] - xdata[id - 1]) *
			                              (ydata[id] - ydata[id - 1]);
...
/*
interpLayer(double frac_in, int n_in, double* xs_in, double* ys_out, int dim_in,
int dim_max      )
{

    for (int ts=0; ts<Nt-1; ts++)			// loop through precalculated wave
time series time steps  (start ts at ts0 to save time) {	if (tTS[ts+1] > t)
// moving precalculated time bracked "up".  Stop once upper end of bracket is
greater than current time t.
        {
            ts0 = ts;
    ...
    for (int i=0; i<n_in; i++)  // loop through passed coordinate values, get
...
interpLayer( ptr ys1[], ptr ys2[], int nin, int n, double *xs, double **pxs
{
    ...
    //          ys1  n-d array slice(s) of y data to be passed to next lower
level
    //          ys2  n-d array slice(s) of y data to be passed to next lower
level  <<< eventually this should be a list of pointers to ALL data needing
interpolation
...
{
	// Parameters: list of x values, number of x values, x value to be
	// interpolated, fraction to return Returns the lower index to interpolate
	// from.  such that  y* = y[i] + fout*(y[i+1]-y[i])
	int i;
...
{
	// This more general/versatile version writes two indices (i, i+1) and two
	// scalers (1-f, f), thereby handling bounds in one step

	// Parameters: list of x values, number of x values, x value to be
	// interpolated, fractions to return, indices to return Such that  y* =
	// y[iout[0]]*fout[0] + y[iout[1]]*fout[1]
...
// convenience function to calculate curvature based on adjacent segments'
// direction vectors and their combined length
moordyn::real
...
void
GetOrientationAngles(double vec[3],
                     double* phi,
                     double* sinPhi,
                     double* cosPhi,
                     double* tanPhi,
                     ...
                     double* cosBeta)
{
	...
	if (vecLen < 0.000001) {
		cout << "ERROR in GetOrientationAngles in MoorDyn. Supplied vector is "
		        "near zero"
		     << endl;
		cout << vec << endl;
	} else {
...
void
scalevector(vector<double>& u, double newlength, vector<double>& y)
{
	double length_squared = 0.0;
	for (int J = 0; J < 3; J++)
		length_squared += u[J] * u[J];
...
void
scalevector(double u[3], double newlength, double y[3])
{
	double length_squared = 0.0;
	for (int J = 0; J < 3; J++)
		length_squared += u[J] * u[J];
	...
	for (int J = 0; J < 3; J++) {
		double temp =
		    u[J] * scaler; // not sure this temp switching is necessary (was
		                   // just if u and y were the same)
		y[J] = temp;
	}
	return;
}
...
// return distance between double arrays r1 and r2   - also in misc.cpp for
// vector types
double
distance3d(double* r1, double* r2)
{
	...
	//	cout << "difference in dimension " << J << " is " << r2[J] << " - " <<
	// r1[J] << endl;
	//}
...
void
inverseM3(vector<vector<double>>& minv, vector<vector<double>>& m)
{
	...
	minv[0][0] =
	    (m[1][1] * m[2][2] - m[2][1] * m[1][2]) * invdet; // cout << minv[0][0];
	minv[0][1] =
	    (m[0][2] * m[2][1] - m[0][1] * m[2][2]) * invdet; // cout << minv[0][1];
	minv[0][2] =
	    (m[0][1] * m[1][2] - m[0][2] * m[1][1]) * invdet; // cout << minv[0][2];
	minv[1][0] =
	    (m[1][2] * m[2][0] - m[1][0] * m[2][2]) * invdet; // cout << minv[1][0];
	minv[1][1] =
	    (m[0][0] * m[2][2] - m[0][2] * m[2][0]) * invdet; // cout << minv[1][1];
	minv[1][2] =
	    (m[1][0] * m[0][2] - m[0][0] * m[1][2]) * invdet; // cout << minv[1][2];
	minv[2][0] =
	    (m[1][0] * m[2][1] - m[2][0] * m[1][1]) * invdet; // cout << minv[2][0];
	minv[2][1] =
	    (m[2][0] * m[0][1] - m[0][0] * m[2][1]) * invdet; // cout << minv[2][1];
	minv[2][2] = (m[0][0] * m[1][1] - m[1][0] * m[0][1]) *
	             invdet; // cout << minv[2][2] << endl;
}
...
RotMat(double x2,
       double x1,
       double x3,
       double TransMat[]) // could rename to EulerToDCM
{
	// note above swapping of x1 and x2 to deal with weird coordinate system
	// from FAST convention ( x2 is roll, x1 is pitch, x3 is yaw )
	...
	// rmat = transpose([ 1 0 0; 0 c1 -s1; 0 s1 c1] * [c2 0 s2; 0 1 0; -s2 0 c2]
	// * [c3 s3 0; -s3 c3 0; 0 0 1]);

	// rmat = [1 rz -ry; -rz 1 rx; ry -rx 1];   % rotation matrix

	// what follows is order Y X Z on
...
//// make a direction cosines matrix from a set of three perpendicular unit
/// vector axes
...
// apply a rotation to a 6-by-6 mass/inertia tensor (see Sadeghi and Incecik
// 2005 for theory)
void
rotateM6(double Min[6][6], double rotMat[3][3], double outMat[6][6])
{
	...
	for (int I = 0; I < 3; I++)
		for (int J = 0; J < 3; J++)
			tempM[I][J] = Min[3 + I][3 + J];
	rotateM3(tempM, rotMat, tempMrotated);
	for (int I = 0; I < 3; I++)
		for (int J = 0; J < 3; J++)
			outMat[3 + I][3 + J] = tempMrotated[I][J];
...
rotateM6(double Min[36], double rotMat[9], double outMat[36])
{
	...
	for (int I = 0; I < 3; I++)
		for (int J = 0; J < 3; J++)
			outMat[18 + 3 * I + J] = outMat[9 + 3 * J + I];
...
// calculate position and velocity of point based on its position relative to
// moving 6DOF body
void
                    ...
                    double rdOut[3])
{
	...
	// locations (unrotated reference frame) about platform reference point
	rRel[0] = TransMat[0] * rRelBody[0] + TransMat[1] * rRelBody[1] +
	          TransMat[2] * rRelBody[2]; // x
	rRel[1] = TransMat[3] * rRelBody[0] + TransMat[4] * rRelBody[1] +
	          TransMat[5] * rRelBody[2]; // y
	rRel[2] = TransMat[6] * rRelBody[0] + TransMat[7] * rRelBody[1] +
	          TransMat[8] * rRelBody[2]; // z
...
transformKinematicsAtoB(double rA[3],
                        ...
                        vector<double>& rdOut)
{
	// rA is coordinate of end A, u is unit vector and L is length along it to
	// end B (which is where outputs are calculated) rd_in should be in global
	// orientation frame note: it's okay if r_out and rd_out are 6-size. Only
	// the first 3 will be written, and 4-6 will
	//       already be correct or can be assigned seperately from r_in and
	//       rd_in (assuming orientation frames are identical)
...
{
	double Moments[3];
	crossProd(dx, F, Moments); // F is actually size 6 but crossProd only looks
	                           // for the first 3 so this is ok
	for (int i = 0; i < 3; i++) {
		Fout[i] = F[i];
		Fout[i + 3] =
		    F[i + 3] +
		    Moments[i]; // add moments from translation to original moments
	}
	return;
}
...
void
transformMass3to6DOF(double PARAM_UNUSED r[3],
                     double PARAM_UNUSED TransMat[9],
                     double PARAM_UNUSED Min[3][3],
                     double PARAM_UNUSED Iin[3][3],
                     double PARAM_UNUSED Mout[6][6])
{
	// r - vector from ref point to CG (center of Min and Iin) in UNROTATED ref
	// frame?
	...
	// see https://aapt.scitation.org/doi/10.1119/1.4994835?? (not it's not very
	// useful)
...
translateMassInertia3to6DOF(double r[3],
                            ...
                            double Mout[6][6])
{
	// Takes in mass and inertia matrices of object with relative CG position r,
	// and calculates 6x6 mass/inertia matrix about current ref point (about
	// which r was made).
...
void translateMass3to6DOF(double r[3], double Min[3][3], double Mout[6][6])
{
    ...
    // calculates 6x6 mass/inertia matrix about current ref point (about which r
was made).
    ...
    // should double check diagonal elements (if I've included everything)	<<
I derived these intuitively... Mout[3][3] = Min[2][2]*yy + m[1][1]*zz Mout[4][4]
= Min[0][0]*zz + m[2][2]*xx; Mout[5][5] = Min[1][1]*xx + m[0][0]*yy;

    Mout[3][4] = -Min[2][2]*xy -Min[0][1]*zz +Min[2][0]*yz +Min[1][2]*zx //xyzz
<<< check signs Mout[4][5] =  Min[0][0]*yz -Min[1][2]*xx +Min[0][1]*zx
+Min[2][0]*xy //xxyz <<< check signs Mout[5][3] = -Min[1][1]*zx -Min[2][0]*yy
+Min[1][2]*xy +Min[0][1]*yz //xyyz Mout[4][3] = Mout[3][4] Mout[5][4] =
Mout[4][5] Mout[5][3] = Mout[5][3]
...
translateMass3to6DOF(double r[3], double Min[3][3], double Mout[6][6])
{
	...
	double H[3]
	        [3]; // "anti-symmetric tensor components" from Sadeghi and Incecik
	getH(r, H);
	...
	// moment of inertia matrix  [I'] = [H][m][H]^T + [J]^T [H] + [H]^T [J] +
	// [I]
	multiplyM3(
	    H,
	    Min,
	    tempM); // could just do a transpose of old tempM here intead.. <<<
...
translateMass6to6DOF(double r[3], double Min[6][6], double Mout[6][6])
{
	...
	for (int I = 0; I < 3; I++) {
		for (int J = 0; J < 3; J++) { // [I']        = [H][m][H]^T  + [J]^T[H]
			                          // + [H]^T[J]    + [I]
			Mout[3 + I][3 + J] =
			    tempM2[I][J] + tempM3[I][J] + tempM4[I][J] + Min[3 + I][3 + J];
		}
	}
	return;
}
...
translateMass6to6DOF(double r[3], double Min[36], double Mout[36])
{
	...
	for (int I = 0; I < 3; I++)
		for (int J = 0; J < 3; J++)
			Mout[27 + 3 * I + J] = tempM2[3 * I + J] + tempM3[3 * I + J] +
			                       tempM4[3 * I + J] + Min[27 + 3 * I + J];
...
vector<string>
splitComma(const string& s) // this one splits at commas
{
	return moordyn::str::split(s, ',');
}
...
doIIR(double* in,
      ...
      double* b,
      int kernelSize)
{ // kernelSize is size of vectors a and b, which contain the coefficients.
  // Normally a[0]=1.
...
// State Space filter function - a good reference is
...
doSSfilter(double* in,
           ...
           double b0,
           int kernelSize)
{ // kernelSize is size of vectors a and beta (exluding 0th entry!), which
  // contain the coefficients.

	int i, k;
	double* fstates =
	    (double*)malloc(dataSize * sizeof(double)); // create state vector
