...
class Rod
{
	...
	// Rod Types:
	//	0: free to move
	//	1: pinned to a fixed point (or a body/ptfm, in which case added to
	// list?) 	2: attached rigidly to a fixed point (or a body/ptfm, in which
	// case added to list?) 	-1: pinned to a coupling point (3dof) 	-2:
	// attached rigidly to a coupling point (6dof)
	...
	moordyn::Line*
	    AttachedA[10]; // pointers to lines attached to this connection node
	moordyn::Line*
	    AttachedB[10]; // pointers to lines attached to this connection node
	int TopA[10]; // which end of line are we attached to? 1 = top/fairlead, 0 =
	              // bottom/anchor
	int TopB[10]; // which end of line are we attached to? 1 = top/fairlead, 0 =
	              // bottom/anchor
	...
	double r6[6]; // Rod 6dof position [x/y/z/u1/u2/u3] (end A coordinates and
	              // direction unit vector)
	double v6[6]; // Rod 6dof velocity[vx/vy/vz/wx/wy/wz] (end A velocity and
	...
	// <<< should rename to be clearly moment not mass
	...
	double Mext[3];  // external moments (from attached cables or waterplane
	                 // hydrostatic moment)
	double F6net[6]; // total force and moment about end A (excluding inertial
	                 // loads) that Rod may exert on whatever it's attached to
	double M6net[6][6]; // total mass matrix about end A of Rod and any attached
	                    // Points
	...
	// wave things
	double* F; // VOF scalar for each segment (1 = fully submerged, 0 = out of
	           // water)
	...
	double t0; // simulation time current integration was started at (used for
	           // BC function)
	double r_ves[6];  // fairlead position for coupled rods [x/y/z]
	double rd_ves[6]; // fairlead velocity for coupled rods [x/y/z]

	// motion component vectors (declaring these here as they caused problems if
	...
	double ap[3]; // transverse component of absolute acceleration - HAD TO MOVE
	              // THIS UP FROM LOWER DOWN (USED TO CRASH AFTER I=3)
	double aq[3]; // axial component of absolute acceleration

	// file stuff
	ofstream* outfile; // if not a pointer, caused odeint system initialization
  ...
  public:
	...
	types type; // 	0: free to move; 1: pinned; 2: attached rigidly (positive if
	            // to something, negative if coupled)
	//	int pinned;      // flag indicating of Rod end A is pinned (1) or free
	//(0/default). Triggered by setting BodyToAddTO to -1.

	double roll;
	double pitch;

	int WaterKin; // flag indicating whether wave/current kinematics will be
	              // considered for this linec
	// 0: none, or use value set externally for each node of the object; 1:
	// interpolate from stored; 2: call interpolation function from global Waves
	// grid
	...
	void initiateStep(const double rFairIn[6],
	...
	void getNetForceAndMass(double rBody[3],
	                        double Fnet_out[6],
	                        double M_out[6][6]);
