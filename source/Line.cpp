...
real Line::getNonlinearE(real l_stretched, real l_unstretched)
{
	if (!nEApoints)
		return EI;
...
real Line::getNonlinearEI(real curv)
{
	if (!nEIpoints)
		return EI;
...
real Line::getNonlinearC(real ld_stretched, real l_unstretched)
{
	if (!nCpoints)
		return c;
	...
	{
		// if no compressing data given, we'll flip-mirror so stretching and compressing are same
		real Xsign = 1.0;
		if (Xi < 0) {
			Xsign = -1.0;
			Xi = -Xi;
...
void Line::setup(int number_in, LineProps *props, real UnstrLen_in,
                 unsigned int NumSegs, shared_ptr<ofstream> outfile_pointer,
                 string channels_in)
{
	number = number_in;
...
void Line::initializeLine(double* X)
{
	LOGMSG << "  - Line" << number << ":" << endl
	       ...
	       << "    Cdn : " << Cdn  << endl
	       << "    Cdt : " << Cdt  << endl
	       << "    ww_l: " << ( (rho - env->rho_w)*(pi/4.*d*d) )*9.81 << endl;
	...
	// (for now)
	if((env->WaveKin == WAVES_FFT_GRID) ||
	   (env->WaveKin == WAVES_GRID) ||
	   (env->WaveKin == WAVES_KIN) ||
	   (env->Current == CURRENTS_STEADY_GRID) ||
	...
	if (UnstrLen < 0)
	{
		...
		LOGMSG << "Line " << number << " unstretched length set to "
		       << UnstrLen << " m" << endl;
	}
	...
	if (-r[0][0] == env->WtrDpth)
	{
		...
		if(( XF > 0.0) && (ZF > 0.0))   
		{
			...
			int success = Catenary(XF, ZF, UnstrLen, E * A, LW, CB, Tol,
			...
			if (success >= 0) {
				...
				LOGWRN << "Catenary initial profile failed for Line "
				       << number << endl;
			}
		}
	}
...
double Line::GetLineOutput(OutChanProps outChan)
{
	...
	else if (outChan.QType == Ten )  return  getNodeTen(outChan.NodeID).norm();
...
void Line::storeWaterKin(unsigned int nt, real dt, const real **zeta_in,
...
void Line::SetEndKinematics(vec pos, vec vel, EndPoints end_point)
{
	switch (end_point) {
		case ENDPOINT_TOP:
			endTypeB = PINNED; // indicate pinned
...
{  	// Note: qin is the rod's position/orientation vector, r6, passed at index 3
	...
	if (topOfLine==1)
	{
		endTypeB = CANTILEVERED;                    // indicate attached to Rod (at every time step, just in case line get detached)
	...
	{	
		endTypeA = CANTILEVERED;                    // indicate attached to Rod (at every time step, just in case line get detached)
...
void Line::setEndOrientation(vec qin, EndPoints end_point,
                             EndPoints rod_end_point)
{
	if ((rod_end_point != ENDPOINT_A) && (rod_end_point != ENDPOINT_B)) {
			LOGERR << "Invalid rod end point qualifier: " << rod_end_point
			       << endl;
			throw moordyn::invalid_value_error("Invalid end point");
	}
	switch (end_point) {
...
vec Line::getEndSegmentMoment(EndPoints end_point, EndPoints rod_end_point) const
{
	real dlEnd, EIEnd;
	vec qEnd;
...
void Line::getStateDeriv(double* Xd, const double PARAM_UNUSED dt)
{
	...
	if ((endTypeA == PINNED) || (EI==0)) unitvector(q[0], r[0  ], r[1]);  
	if ((endTypeB == PINNED) || (EI==0)) unitvector(q[N], r[N-1], r[N]);
	
		
	//============================================================================================
	// --------------------------------- apply wave kinematics -----------------------------
	
	if (WaterKin == WAVES_EXTERNAL) // wave kinematics time series set internally for each node
	{
	...
	else if (WaterKin == WAVES_GRID)
	{
		...
		for (unsigned int i = 0; i <= N; i++)
		{
			...
			waves->getWaveKin(r[i][0], r[i][1], r[i][2], t, U[i], Ud[i],
			                  zeta[i], PDyn[i]);

			F[i] = 1.0; // set VOF value to one for now (everything submerged - eventually this should be element-based!!!) <<<<
		}
	}
	else if (WaterKin != WAVES_NONE) // Hopefully WaterKin is set to zero, meaning no waves or set externally, otherwise it's an error
	...
	if (EI > 0)
	{
		...
		for (unsigned int i = 0; i <= N; i++)
		{
			...
			if (i==0)
			{
				if (endTypeA == CANTILEVERED) // if attached to Rod i.e. cantilever connection
				{
			...
			{
				if (endTypeB == CANTILEVERED) // if attached to Rod i.e. cantilever connection
				{
	...
	for (unsigned int i=0; i<=N; i++)
	{
		...
		if (r[i][2] < -env->WtrDpth)
		{
			...
			real FrictionMax = abs(B[i][2])*env->FrictionCoefficient; // dynamic friction force saturation level based on bottom contact force
			
			// saturated damping approach to applying friction, for now
			real BottomVel = sqrt(rd[i][0]*rd[i][0] + rd[i][1]*rd[i][1]); // velocity of node along sea bed
			real FrictionForce = BottomVel * env->FrictionCoefficient*env->FricDamp; // some arbitrary damping scaling thing at end
